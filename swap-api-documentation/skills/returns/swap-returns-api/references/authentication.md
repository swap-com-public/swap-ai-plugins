...
**Otherwise**, read `~/.swap-api.json` and look up `["returns"][environment]` to get the headers object. If any value starts with `$`, resolve it from that environment variable at call time.
...
- **In:** header
- **Name:** X-API-Key
- **Config path:** `~/.swap-api.json` → `returns` → environment → `X-API-Key`
