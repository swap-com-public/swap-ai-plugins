# GET /v1/agents/stores/{swapId}/size-categories

**Resource:** [SizeCategory](../resources/SizeCategory.md)
**Size categories for catalog sizing UX**
**Operation ID:** `SizeCategoryController_getSizeCategories_v1`

## Parameters

| Name | In | Type | Required | Description |
|------|------|------|----------|-------------|
| `swapId` | path | string | Yes |  |
| `x-public-key` | header | string | Yes | Client public API key from onboarding. |

## Responses

| Status | Description |
|--------|-------------|
| 200 | Size categories fetched successfully. |
| 400 | Bad Request — invalid path/query/body or failed pipe validation. |
| 401 | Unauthorized — missing or invalid credentials (gateway or downstream). |
| 403 | Forbidden — insufficient permissions for this resource. |
| 404 | Not Found — resource does not exist or is not visible to the caller. |
| 422 | Unprocessable Entity — semantic validation failed (e.g. checkout session state). |
| 429 | Too Many Requests — gateway rate limit. |
| 500 | Internal Server Error — unexpected failure (see `message`; may be sanitized in prod/staging). |
| 502 | Bad Gateway — gateway could not complete the proxied request to a downstream service. |

**Success Response Schema** (inline):

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `messages` | string[] | Yes |  |
| `data` | [SizeCategoryResponse](../schemas/Size/SizeCategoryResponse.md) | Yes |  |

## Security

- **ClientPublicKey**
